	...
	<div id="sidebar">
		<ul>
			...
			<li><a href="javascript:showPoolUsedDisks()"><i
					class="icon-hdd"></i> <span>节点磁盘管理</span></a></li>
		</ul>
	</div>
			...
			<div class="container-fluid">
				<c:forEach var="diskCount" items="${diskCount}" varStatus="status">
					<c:if test="${diskCount.key == 'FREE'}">
						<span class="label label-info">未使用：${diskCount.value}</span>
					</c:if>
					<c:if test="${diskCount.key == 'USED'}">
						<span class="label label-success">使用中：${diskCount.value}</span>
					</c:if>
					<c:if test="${diskCount.key == 'FAIL'}">
						<span class="label label-important">故障中：${diskCount.value}</span>
					</c:if>
				</c:forEach>
			</div>
		</div>
							...
							<div class="span6">
								<div class="widget-box collapsible">
									<div class="widget-title">
										...
										</a> <span class="label label-inverse">未知</span>
											<span class="label">未连接</span> <span
											class="label label-success">使用中</span> <span
											class="label label-important">故障中</span> <span
											class="label label-info">未使用</span>
									</div>
