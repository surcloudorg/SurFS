...
public final class MD5Digest extends GeneralDigest {
    ...
    private static final long serialVersionUID = 20140734434456L;
    ...
    public byte[] toBytes() {
        ...
        byte[] bs = new byte[len + 4 * 4 + 4 * 16 + 1];
        ...
        int2byte(X, 0, bs, len + 4 * 4, 16);
        bs[len + 4 * 4 + 4 * 16] = xOff;
        return bs;
    }
    ...
    protected void fill(byte[] bs) {
        ...
        byte2int(bs, len + 4 * 4, X, 0, 16);
        xOff = bs[len + 4 * 4 + 4 * 16];
    }
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
        byte[] bs = new byte[byte_length + 10 + 4 * 4 + 4 * 16 + 1];
        in.read(bs);
        fill(bs);
    }
     ...
     * @param algorithm
     */
    protected MD5Digest(String algorithm) {
        super(algorithm);
        reset();
    }
